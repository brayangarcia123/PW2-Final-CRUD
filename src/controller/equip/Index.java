...
public class Index extends HttpServlet {
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		if(user == null){
			...
			Query query = pm.newQuery(Equip.class);
			//query.setOrdering("nombre descending");
			List<Equip> array = (List<Equip>)query.execute("select from Equip");

			req.setAttribute("array", array);
			try {
				req.getRequestDispatcher("/WEB-INF/Views/Equip/Index.jsp").forward(req, resp);
				query.closeAll();

...
public class Index extends HttpServlet {
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		List<Access> listAcceso = (List<Access>)query0.execute("select from Access");
		
		List<AccessAux> array = new ArrayList<AccessAux>();
		AccessAux nuevo;
		...
		for(Access acceso:listAcceso){
			...
			nuevo = new AccessAux(acceso.getId(),rol.getNombre(),recurso.getUrl(),acceso.isEstado());
			array.add(nuevo);
		}

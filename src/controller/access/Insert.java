...
public class Insert extends HttpServlet {
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		List<Resource> arrayRecurso = (List<Resource>)query1.execute("select from Resource");
		
		
		try {
			req.setAttribute("arrayRole",arrayRole);
			req.setAttribute("arrayRecurso",arrayRecurso);
			req.getRequestDispatcher("/WEB-INF/Views/Access/Insertar.jsp").forward(req, resp);
